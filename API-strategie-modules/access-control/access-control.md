...
The [NL GOV Assurance profile for OAuth 2.0](https://publicatie.centrumvoorstandaarden.nl/api/oauth/) supports Client Credentials. The [Forum Standaardisatie Comply-Or-Explain-List](https://www.forumstandaardisatie.nl/open-standaarden/nl-gov-assurance-profile-oauth-20) states when this profile has to be used: 
The NL GOV Assurance Profile for OAuth 2.0 must be applied to applications where users or resource owners implicitly or explicitly consent to a third-party service to access data via a REST API for which they have access rights.
...
Both NL GOV Assurance Profile for OAuth 2.0 and Federated Service Connectivity (FSC) are relevant standards for access control.
...
* 1 In the use case a User/Resource owner gives consent to a third-party to access Data
    ...
    * NO → go to step 2
*  2 In the use case there is data exchange with central systems that are part of the GDI (General Digital Infrastructure (for example base registries) or the data exchange is cross-sectoral). Also there is a need for two way authentication.
    * YES → Digikoppeling REST-API profile with FS
