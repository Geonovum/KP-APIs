...
![De plaats van API's bij aanbod van diensten](https://github.com/Geonovum/KP-APIs/raw/master/media/clip02.png)
...
- (A) : API Koppelvlak Overheid naar Burgers en Bedrijven
- (B) : API Koppelvlak Overheid naar Overheid (bv onderdelen GDI)

Toelichting API doelgroepen:
...
(Bijvoorbeeld API’s gebaseerd op de Generieke Functies (capabilities) van de Nederlandse Overheid, zie [de generieke functies van noraonline.nl](https://www.noraonline.nl/wiki/Generieke_functies), en op de capabilities van de eigen organisatie).
...
- Design Philosophy (eg RESTful, GraphQL, Event Driven, Orkestratie)
- Communications Protocol (eg HTTP, Websockets)
- Encoding (eg JSON, Cloudevents, Protobuf (binary))
...
- Event-based Style: Cloudevents, MQ, WebSub, MQTT, XMPP, AMQP, Kafka, AsyncAPI
...
> \* Verder aangeduid als 'integratie', van ook bijvoorbeeld informatie.
...
![Functionaliteit binnen API-Management](https://github.com/Geonovum/KP-APIs/raw/master/media/API-Management.jpg)

> Figuur 6: Functionaliteit binnen API-Management
...
![Resource Model](https://github.com/Geonovum/KP-APIs/raw/master/overleggen/Werkgroep%20API%20architectuur/uitwerkingen/media/API-Informatiemodel.png)

> Figuur 7: Resource Model
...
![Geïntegreerd Resource Model](https://github.com/Geonovum/KP-APIs/raw/master/overleggen/Werkgroep%20API%20architectuur/uitwerkingen/media/API-Infomodel2.png)

> Figuur 8: Geïntegreerd Resource Model
...
Bovenstaand figuur beschrijft de basis van de API Security architectuur. Binnen de Nederlandse overheid zijn meerdere niveau's van beveiliging (zie ook de BIO 2.0) en authenticatie dan in reguliere commerciële situaties. Denk hierbij bijvoorbeeld aan het redundant aanbieden van API toegang vis een besloten netwerk als Diginetwerk of het afdwingen van mTLS en authenticatie met een PKIO certificaat om er zo zeker van te zijn dat de client ook echt van een identificeerbare en erkende organisatie is.

Daarnaast zijn er ook nog flows om tokens uit te geven aan gebruikers op basis van OAuth / OIDC en diepgaandere technieken zoals Externalized Authorization Management (EAM) om centraal beheerde policies met toegangsregels af te dwingen met PBAC op basis van AuthZen. Tot slot is het voor uitwisseling van gegevens tussen overheidsorganisaties ook nog aan te bevelen de FSC standaarden toe te passen voor G2G verkeer.

Het spreekt voor zich dat niet al deze security aspecten gestapeld moeten worden toegepast op een API en dat er per situatie en doelgroep een risico analyse en bijbehorende maatregelen moeten worden toegepast. De module Access Control geeft hierin meer context en richting.
...
- [Federatieve Toegangsverlening (FTV)](https://vng-realisatie.github.io/ftv/)
- [Federatieve Service Connectiviteit (FSC)](https://fsc-standaard.nl/)
- [Open Authenticatie (OAUTH)](https://gitdocumentatie.logius.nl/publicatie/api/oauth/)
- [OpenID Connect (OIDC)](https://gitdocumentatie.logius.nl/publicatie/api/oidc/)
- [Module Access control](https://docs.geostandaarden.nl/api/API-Strategie-mod-access-control/)
...
Er zijn reeds enkele voorbeelden van use cases waarbij orkestratie een belangrijke rol speelt:
...
- [Diverse Digital Twin use cases](https://www.geonovum.nl/themas/digital-twins)
...
Voor een succesvolle orkestratie is het van belang dat alle gateway's, die de api's beschermen die in de orkestratie worden aangeroepen, op een eenduidige en voorspelbare manier de policies afdwingen. Bij het opzetten van een Orkestratie is het dan ook niet alleen van belang om aandacht te geven aan Data integriteit en Authenticatie/autorisatie maar indien er op de API's ook Policy Based Access Control (PBAC) of Externalized Authorization Management (EAM) wordt toegepast moeten deze aspecten als aandachtspunt worden meegenomen.

Overweeg bij het aanbieden van een proces api als interface voor een orkestratie dat het belangrijk is dat alle policies van de system api's optellen en deze policies ook afgedwongen moeten worden in de proces api.

Het Federatief Datastelsel (FDS) heeft hiervoor verschillende faciliterende stelselfuncties benoemd.
...
Kijk voor een overzicht van alle Stelselfuncties van het Federatief Datastelsel op de [Stelselfuncties pagina.](https://federatief.datastelsel.nl/kennisbank/stelselfuncties/)
